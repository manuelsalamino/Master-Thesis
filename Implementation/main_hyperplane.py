...
files = ['Breastw.csv']
#files = ['Http.csv', 'ForestCover.csv', 'Mulcross.csv', 'Shuttle.csv', 'Smtp.csv', 'hbk.csv', 'wood.csv']
...
for dataset_name in files:
    ...
    centroid = get_data_centroid(ifor.histogram_data[training_indexes])

    # pca parameters (fit data)
    pca = PCA(n_components=1)
    pca.fit(ifor.histogram_data[training_indexes])
    par = pca.components_[:1]

    # IFOR anomaly score parameters (avg of depths of the trees)
    #par = [[i for i in range(1, centroid.shape[0]+1)]]

    # put together all the parameters (centroid is the note term, the others are computed by fitting or taken from IFOR)
    parameters = np.concatenate([[centroid], par], axis=0).reshape(2, centroid.shape[0])
    ...
    roc_auc_IFOR_test, roc_auc_embedding_test = ifor.roc_auc_along_hyperplane(indexes=test_indexes, parameters=parameters)

    print('Train/Test split:\n',
          'roc_auc_IFOR: ', roc_auc_IFOR_test,
          'roc_auc_embedding: ', roc_auc_embedding_test)
    ...
    roc_auc_IFOR_entire, roc_auc_embedding_entire = ifor.roc_auc_along_hyperplane(indexes=np.arange(dataset.n_samples),
                                                                    parameters=parameters)
    print('\nEntire Dataset:\n',
          'roc_auc_IFOR: ', roc_auc_IFOR_entire,
          'roc_auc_embedding: ', roc_auc_embedding_entire)
    ...
    df = pd.DataFrame([[ifor.dataset.dataset_name, roc_auc_IFOR_test, roc_auc_embedding_test,
                        roc_auc_IFOR_entire, roc_auc_embedding_entire]],
                      columns=['dataset', 'roc_auc_IFOR_test', 'roc_auc_embedding_test',
                               'roc_auc_IFOR_entire', 'roc_auc_embedding_entire'])
